...
def identify_timeframes_communication(broker_name):
    ...
    for apprehend_key, comm_cfg in tf_comm_section.items():
        ...
        for sym in sorted(os.listdir(base_folder)):
            ...
            for s_tf, r_tf in zip(sender_tfs, receiver_tfs):
                ...
                # Pattern to match all files generated by identify_timeframes_communication
...
def identify_receiver_comm_higher_highs_higher_lows(broker_name):
    lagos_tz = pytz.timezone('Africa/Lagos')
    ...
    for apprehend_key, comm_cfg in tf_comm_section.items():
        ...
        for sym in sorted(os.listdir(base_folder)):
            ...
            for s_tf, r_tf in zip(sender_tfs, receiver_tfs):
                # Use get_analysis_paths to get the output directory
                paths = get_analysis_paths(
                    base_folder, broker_name, sym, s_tf, direction, bars, 
                ...
                # Pattern to match all communication files for this sender → receiver pair
                # Example: 15m_higherhighsandhigherlows_contourmaker_4h_*.json
                base_pattern = f"{r_tf}_{source_filename.replace('.json','')}_*_{s_tf}_*"
                ...
                for json_path in json_files:
                    # Construct corresponding PNG path
                    ...
                    try:
                        ...
                        for i in range(neighbor_left, n - neighbor_right):
                            ...
                            # Mark the contour maker (confirmation candle to the right)
                            m_idx = i + neighbor_right
                            ...
                            if m_idx < n:
                                ...
                                contour_maker_entry = data[m_idx]

                            enriched_node = data[i].copy()
                            enriched_node.update({
                                "type": "higher_low" if is_bull else "higher_high",
                                "contour_maker": contour_maker_entry,
                                "active_color": active_color
                            })
                            swing_results.append(enriched_node)
                        ...
                        if swing_results:
                            ...
                            log(f"Processed {os.path.basename(json_path)} → {len(swing_results)} swings marked", "INFO")

                    except Exception as e:
                        log(f"Error processing {sym} | {os.path.basename(json_path)}: {e}", "ERROR")

    return f"Identify Receiver Comm Done. Processed {processed_charts_all} files with total {total_marked_all} swings marked."
...
def single():
    ...
    with Pool(processes=cores) as pool:
        ...
        hh_hl_results = pool.map(identify_directional_bias, broker_names)
...
def main():
    ...
    with Pool(processes=cores) as pool:
        ...
        tf_comm_results = pool.map(identify_timeframes_communication, broker_names)
        ...
        hh_hl_results = pool.map(identify_receiver_comm_higher_highs_higher_lows, broker_names)
        for r in hh_hl_results: print(r)
