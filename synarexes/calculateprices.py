...
def load_global_symbol_data():
    def normalize_symbol(s):
        return " ".join(str(s).replace("_", " ").split()).upper() if s else ""
    ...
    if os.path.exists(DICTATOR_PATH):
        try:
            with open(DICTATOR_PATH, 'r', encoding='utf-8') as f:
                ...
                for cat, syms in dictator.items():
                    for s in syms:
                        norm = normalize_symbol(s)
                        if norm:
                            symbol_to_category[norm] = cat
        except Exception as e:
            print(f"[WARNING] Failed to load symbolscategory.json: {e}")
        ...
        try:
            with open(SYMBOLSTICK_PATH, 'r', encoding='utf-8') as f:
                ...
                for key, info in raw.items():
                    ...
                    broker_clean = ''.join(c for c in str(info.get("broker", "")) if not c.isdigit()).lower()
                    symbolstick_data[norm] = {
                        "broker": broker_clean,
                        "tick_size": info.get("tick_size", 0.00001),
                        "tick_value": info.get("tick_value", 0.0)
                    }
        except Exception as e:
            print(f"[WARNING] Failed to load symbolstick.json: {e}")

    return symbol_to_category, symbolstick_data
SYMBOL_TO_CATEGORY, SYMBOLSTICK_DATA = load_global_symbol_data()
...
def scale_orders_proportionally():
    ...
    try:
        with open(BROKERS_JSON_PATH, 'r', encoding='utf-8') as f:
            developersdictionary = json.load(f)
...
def check_risk_integrity():
    ...
    BASE_DEVELOPERS_DICT = r"C:\xampp\htdocs\chronedge\synarex\developersdictionary.json"
...
def calculate_forex_sl_tp_market():
    ...
    for user_brokerid, items in data.items():
        ...
        for temp_path in temp_files:
            ...
            # Clean existing forex output files (to ensure fresh calculation every run)
            for risk_folder in calc_risk_dir.iterdir():
                if risk_folder.is_dir() and risk_folder.name.startswith("risk_") and risk_folder.name.endswith("_usd"):
                    for forex_file in risk_folder.glob("*_forex.json"):
                        try:
                            forex_file.unlink()
                        except:
                            pass
            ...
            forex_config = asv_data.get("forex", {})

            # NEW LOGIC:
            # - If limited == False → allow ALL forex symbols found in the temp file (ignore "allowed" list)
            # - If limited == True → allow only the symbols explicitly listed in "allowed"
            limited = forex_config.get("limited", False)
            allowed_list = forex_config.get("allowed", [])

            if limited and not allowed_list:
                # limited=True but no symbols listed → treat as no forex allowed
                continue

            # Build symbol_rules dictionary
            symbol_rules = {}
            if limited:
                # Whitelist mode: only listed symbols
                for item in allowed_list:
                    sym = " ".join(item.get("symbol", "").replace("_", " ").split()).upper()
                ...
                # Allow-all mode: we will create rules dynamically for any forex symbol we encounter later
                pass

            if limited and not symbol_rules:
                continue

            # Load *_accountmanagement.json
            ...
            try:
                rr_ratio_raw = settings.get("risk_reward_ratio", 1.0)
            ...
            for market_key, market_data in orders_section.items():
                if market_key in ["total_orders", "total_markets"] or market_data.get("category", "").lower() != "forex":
                    continue

                forex_found = True
                norm_market = " ".join(market_key.replace("_", " ").split()).upper()

                # Determine if this symbol is allowed
                if limited:
                    ...
                    # limited=False → allow all, but we need default volume/risk
                    # You may adjust these defaults as needed
                    volume_to_use = 0.01   # default volume
                    risk_usd_to_use = 4.0  # default risk in USD
                    # Optionally you could make these configurable elsewhere

                tick_info = SYMBOLSTICK_DATA.get(norm_market, {})
                if tick_info.get("broker") != broker_clean:
                    tick_info = {}
                tick_size = tick_info.get("tick_size", 0.00001)
                tick_value = tick_info.get("tick_value", 0.0)
            ...
            # Rest of the function remains unchanged (filtering, coordination, saving, etc.)
            ...
            for norm_market, orders in filtered_symbol_orders.items():
                if not orders:
                    continue
                sample = orders[0]
                digits = 5 if sample["tick_size"] <= 1e-5 else 3
            ...
            if not forex_found or not results_by_risk:
                continue

            # Save to risk folders (always fresh)
            for risk_usd, markets_dict in results_by_risk.items():
                ...
                out_file = risk_folder / f"{original_stem}_forex.json"
                try:
                    with open(out_file, 'w', encoding='utf-8') as f:
                        json.dump(orders_structure, f, indent=2, ensure_ascii=False)
        ...
        # Minimal per-broker output only
        if files_saved_this_broker > 0:
            print(f" → Generated {files_saved_this_broker} JSON file(s)\n")
        else:
            print(" → No output files generated\n")
